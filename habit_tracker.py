...
def open_tag_manager():
    ...
    window_two.title("Tag Manager")
    window_two.geometry("600x700")

    main_frame_two = tk.Frame(window_two)
    main_frame_two.pack(fill="both", expand=True)

    canvas_two = tk.Canvas(main_frame_two)
    canvas_two.pack(side="left", fill="both", expand=True)

    ver_scroll = tk.Scrollbar(main_frame_two, orient="vertical", command=canvas_two.yview)
    ver_scroll.pack(side="right", fill="y")

    canvas_two.configure(yscrollcommand=ver_scroll.set)
    
    content_frame = tk.Frame(canvas_two)
    window_id = canvas_two.create_window((0, 0), window=content_frame, anchor="nw")

    def on_canvas_configure(event):
        canvas_two.itemconfig(window_id, width=event.width)

    canvas_two.bind("<Configure>", on_canvas_configure)

    def on_content_configure(event):
        canvas_two.configure(scrollregion=canvas_two.bbox("all"))

    content_frame.bind("<Configure>", on_content_configure)

    def _on_mousewheel(event):
        canvas_two.yview_scroll(int(-1*(event.delta/120)), "units")

    canvas_two.bind_all("<MouseWheel>", _on_mousewheel)

    tag_listbox = tk.Listbox(content_frame, height=15, selectmode="single")
    tag_listbox.pack(fill="both", expand=True, padx=10, pady=5)
    ...
    def delete_selected_tag():
        selected_index = tag_listbox.curselection()
        if not selected_index:
            messagebox.showwarning("No Selection", "Please select a tag to delete.")
            return
    
        tag_to_delete = tag_listbox.get(selected_index[0])
    ...
    tk.Button(content_frame, text="Delete Tag", command=delete_selected_tag).pack(pady=5)
    
    def edit_selected_tag():
        selected_index = tag_listbox.curselection()
        if not selected_index:
            messagebox.showwarning("No Selection", "Please select a tag to edit.")
            return
        old_tag = tag_listbox.get(selected_index[0])
    ...
    tk.Button(content_frame, text="Edit Tag", command=edit_selected_tag).pack(pady=5)